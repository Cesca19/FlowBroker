...
if ! conan install . -s compiler.cppstd=gnu17 -s build_type=${BUILD_TYPE} -o isShared=${IS_SHARED} --build=missing -c tools.system.package_manager:mode=install -c tools.system.package_manager:sudo=True; then
